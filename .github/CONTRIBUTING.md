Contributing to EToD Engine!
======================

Thanks for your interest in contributing to EToD Engine! :tada: We love getting [pull requests](https://www.quora.com/GitHub-What-is-a-pull-request) for bugfixes and contributions of our community to keep EToD Engine growing.
...
SlaterVanS Studios use English for project management and development, the devs ask to keep the code base, issues and pull requests in English only.
...
You've created a new fix or feature for EToD Engine. Awesome!

1. If you haven't already, create a fork of the EToD Engine repository.
...
4. Give us a moment. EToD Engine is maintained by only a few people, all of whom are doing this on their limited free time, so it may take us a bit to review your request. Bug fixes should be merged in directly, while features usually require SlaterVanS Studios approval with or without it mentioned in one (or more) article.
...
- Naming convention:
  ...
  - For macros we use snake case: **`MACRO_NAME`**.
    - If it is specifically related to EToD Engine, we add the 'ETOD_' prefix: **`ETOD_MACRO_NAME`**.
    - If there is a macro for the application and for the engine, we add an additional 'CORE_' prefix to the engine macro:  **`ETOD_CORE_MACRO_NAME`**.
...
[issue-tracker]: https://github.com/SlaterVanS/EToD/issues
[submit-pr]: https://github.com/SlaterVanS/EToD/pulls
