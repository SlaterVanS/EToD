...
namespace ETOD {
	...
	static std::unordered_map<std::string, ScriptFieldType> s_ScriptFieldTypeMap =
	{
		...
		{ "ETOD.Vector2", ScriptFieldType::Vector2 },
		{ "ETOD.Vector3", ScriptFieldType::Vector3 },
		{ "ETOD.Vector4", ScriptFieldType::Vector4 },

		{ "ETOD.Entity", ScriptFieldType::Entity },
	};
	...
	namespace Utils {
		...
		void PrintAssemblyTypes(MonoAssembly* assembly)
		{
			...
			for (int32_t i = 0; i < numTypes; i++)
			{
				...
				ETOD_CORE_TRACE("{}.{}", nameSpace, name);
			}
		}
		...
		ScriptFieldType MonoTypeToScriptFieldType(MonoType* monoType)
		{
			...
			{
				ETOD_CORE_ERROR("Unknown type: {}", typeName);
	...
	void ScriptEngine::LoadAssemblyClasses()
	{
		...
		int32_t numTypes = mono_table_info_get_rows(typeDefinitionsTable);
		MonoClass* entityClass = mono_class_from_name(s_Data->CoreAssemblyImage, "ETOD", "Entity");
		...
		for (int32_t i = 0; i < numTypes; i++)
		{
			...
			ETOD_CORE_WARN("{} has {} fields:", className, fieldCount);
			...
			while (MonoClassField* field = mono_class_get_fields(monoClass, &iterator))
			{
				...
				if (flags & FIELD_ATTRIBUTE_PUBLIC)
				{
					...
					ETOD_CORE_WARN("  {} ({})", fieldName, Utils::ScriptFieldTypeToString(fieldType));
